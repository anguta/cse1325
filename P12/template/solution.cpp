...
/*int main() {
    std::vector<Top3> v {13, 27, 48, -9, -888, 42, 36, 11};
    Top3 t3;
    for(auto& e : v) {
        std::cout << e << " ";
        t3.add(e);
...
class Triple {
  ...
  public:
    ...
    friend std::ostream& operator<<(std::ostream& ost, const Triple& triple) {
        ost << "(" << triple._x << "," << triple._y << "," << triple._z << ")";
        return ost;
    }
    // did need help from suggested solution for class Triple  
    // but I now understand why
    bool operator<(const Triple& triple) {
        return magnitude() < triple.magnitude();
    }
