...
public class WordSearch {
    ...
    public void solve() {
        ...
        int dividePuzzles = NUM_PUZZLES / NUM_THREADS;
        // used if not evenly divisible
        int lostPuzzles = NUM_PUZZLES % NUM_THREADS;
        int firstPuzzle = 0;
        
        Thread[] threadID = new Thread[NUM_THREADS];
        
        for(int i = 0; i < NUM_THREADS; i++) {
             int lastPuzzlePlusOne = firstPuzzle + dividePuzzles;
             // adds lost puzzles from dividing, if needed
             if(i < lostPuzzles) { lastPuzzlePlusOne++; }
             
             final int ThreadID = i;
             final int FirstPuzzle = firstPuzzle;
             final int LastPuzzlePlusOne = lastPuzzlePlusOne;
             
             threadID[i] = new Thread(() -> {
                 solve(ThreadID, FirstPuzzle, LastPuzzlePlusOne);
             }); threadID[i].start();
             
             firstPuzzle = lastPuzzlePlusOne;
        }
